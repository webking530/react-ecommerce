...
(function($) {
  $.fn.drilldownSelect = function(params) {
    ...
    var defaults = {
      ...
      textBack: Lang.get('js_messages.merchant.back'), // text for go back link
    ...
    window.setItemSelected = this.setItemSelected;
    return this;
    ...
    function makeDropdown(element, event, path, show) {
      ...
      for (var i = 0; i < data.length; i++) {
        ...
        var chieldItemElement = '<li class="' + hasChild + ' ' + disabled +'">' + 
                                    '<a href="#" id="cat_' + dataItem[defaults.keyName] + '" data-path="' + tempPathArray.join(',') + '" data-id="' + dataItem[defaults.keyName] + '">' + dataItem[defaults.valueName] + disabledText + '</a>' +
                                '</li>';
