...
class LocalImageHandler implements ImageHandlerInterface
{
	...
	public function upload($image, $options = [])
	{
        ...
        if(isset($options['compress_size']) && count($options['compress_size']) > 0) {
        	foreach ($options['compress_size'] as $compress_size) {
        		$this->compress_image($dir_name."/".$file_name, $dir_name."/".$file_name, 80, $compress_size['width'],$compress_size['height']);
        	}
        }
