...
<main id="site-content" class="p-0 cls_categoryview" role="main">
	<div class="container-fluid">
		<div class="cls_bannerimg">
			<img src="{{ url('image/homepage/'.$recommended->image) }}">
			<h2>{{$recommended->title}}</h2>
		</div>
	</div>
	<div class="slide-listing text-center pt-4">
					...
					<div class="min-size short-padding">
						...
						<div class="img-content pro_likes">
							<span ng-if='all.user_like.length'>
								<button class="btn-like product_like {{ (!Auth::id()) ? 'without_login' : ''}} {{ (@Auth::user()->status != 'Inactive') ? 'active-user' : 'inactive-user'}}" ng-click= 'product_like(all,all.id)' id= '@{{ all.id }}'  ng-cloak ><span ><i ng-bind="@{{ all.like_user.length}}"></i></span>
								</button>
							</span>
							<span ng-if='!all.user_like.length '>
								<button class="btn-gray product_like {{ (!Auth::id()) ? 'without_login' : ''}} {{ (@Auth::user()->status != 'Inactive') ? 'active-user' : 'inactive-user'}}" ng-click= 'pdu_like(all,all.id)' id= '@{{ all.id }}' ng-as=@{{all}}><span><i ng-bind="@{{ all.like_user.length}}"></i></span >
								</button>
							</span>
